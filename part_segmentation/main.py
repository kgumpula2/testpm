...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            with torch.no_grad():
                cur_pred_val = pred_choice.view(points.size()[0], -1).cpu().data.numpy()
                target = target.view(points.size()[0], -1).cpu().data.numpy()
        ...
        with torch.no_grad():
            ...
            mean_shape_ious = np.nanmean(list(shape_ious.values()))
            ...
            test_metrics['inctance_avg_iou'] = np.nanmean(all_shape_ious)
            log_string('Class avg mIOU: %f' % (test_metrics['class_avg_iou']))
            print('Epoch %d  Class avg mIOU: %f   Inctance avg mIOU: %f' % (
                epoch + 1, test_metrics['class_avg_iou'], test_metrics['inctance_avg_iou']))
